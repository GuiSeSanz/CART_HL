...
CD4_signature_genes <-read.delim(file="/home/mecc/clustermecc/RNA_HighLow_ALL/figures/March_both_HL/BatchK_CD4_SIGGenes_BASAL_BOTH_LowvsHigh.tsv",sep="\t",header=T)
CD8_signature_genes <-read.delim(file="~/clustermecc/RNA_HighLow_ALL/figures/March_both_HL/BatchK_CD8_SIGGenes_BASAL_BOTH_LowvsHigh.tsv",sep="\t",header=T)

CD4_signature <- as.list(CD4_signature_genes)
CD8_signature <- as.list(CD8_signature_genes)

CD4_signature<-read.delim(file="~/clustermecc/RNA_HighLow_ALL/figures/March_both_HL/BatchK_CD4_output_BASAL_BOTH_LowvsHigh.tsv",sep="\t",header=T)
CD8_signature<-read.delim(file="~/clustermecc/RNA_HighLow_ALL/figures/March_both_HL/BatchK_CD8_output_BASAL_BOTH_LowvsHigh.tsv",sep="\t",header=T)
...
exprMatrix_cd4<-read.csv(file="/home/mecc/Desktop/ForGuille_exprMatrix_cd4.csv")
exprMatrix_cd8<-read.csv(file="/home/mecc/Desktop/ForGuille_exprMatrix_cd8.csv")
cd4_metadata <- read.csv(file="/home/mecc/Desktop/ForGuille_metadata_cd4.csv", row.names = "X")
cd8_metadata <- read.csv(file="/home/mecc/Desktop/ForGuille_metadata_cd8.csv", row.names = "X")
...
ann_markers <- inner_join(x = zcore,
                          y = res2.cd4[, c("GeneID","log2FoldChange", "padj")],by = c("GeneID" = "GeneID")) %>% unique()

head(ann_markers[, 1:5])
head(ann_markers[, ncol(ann_markers)-4:ncol(ann_markers)])
head(ann_markers[, 12860:12864])
...
DT.cd4.high %>% pivot_longer(!GeneID, names_to= "cells", values_to = "result" )
DT.cd4.high %>%  
...
cd4_metadata <- inner_join(x = DF.DT.cd4.high [, c("cells", "High pondered")],
...
DT.cd4.low <- DT.low %>% bind_rows(summarise(.,
                                               ...
                                               across(where(is.character), ~"Total")))
tail(DT.cd4.low[,1:5])
DT.cd4.low %>% pivot_longer(!GeneID, names_to= "cells", values_to = "result" )
DT.cd4.low %>%  
  dplyr::filter(GeneID  == "Total") %>% 
  pivot_longer(!GeneID, names_to= "cells", values_to = "result" ) %>% 
  ggplot(aes(x = result)) + geom_histogram (bins=50)
...
cd4_metadata <- inner_join(x = DF.DT.cd4.low [, c("cells", "Low pondered")],
...
#####
car_exp <- scale(cd4_metadata$`High pondered`)
set.seed(123)
...
#####
car_exp <- scale(cd4_metadata$`Low pondered`)
set.seed(123)
car_p33 <- quantile(car_exp[car_exp>0], probs = c(0.25))
set.seed(123)
car_p66 <- quantile(car_exp[car_exp>0], probs = c(0.75))
